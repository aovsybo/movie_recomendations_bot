...
class TextAnalyseRatingTest(TestCase):
    def test_rating_start_end(self):
        message = "Хочу фильм с рейтингом от 3.5 до 6"
        rating = "3.5-6.0"
    ...
    def test_rating_start(self):
        ...
        rating = f"4.0-{settings.MAX_KP_RATING}"
        self.assertEqual(text_analyse(message)["rating.kp"], rating)

    def test_rating_end(self):
        message = "Какой сериал в жанре драма посмотреть с рейтингом меньше 9.1"
        rating = f"{settings.START_KP_RATING}-9.1"
        self.assertEqual(text_analyse(message)["rating.kp"], rating)
